...
zipSubmit.addEventListener("click", (event: Event) => {
  event.preventDefault();
  const zipcode: string = (document.getElementById("zipcode") as HTMLInputElement).value;
  let fetchURL: string = `https://api.openweathermap.org/data/2.5/weather?zip=${zipcode},us&appid=${API_Key}&units=imperial`;
  fetch(fetchURL)
    .then(response => response.json())
    .then(data => {
      let fetchData: string = `https://api.openweathermap.org/data/2.5/onecall?lat=${data.coord.lat}&lon=${data.coord.lon}&appid=${API_Key}&units=imperial`;
...
zipcodeEnter.addEventListener("keydown", (event: KeyboardEvent) => {
  if (event.key === "Enter") {
    const zipcode: string = zipcodeEnter.value;
    let fetchURL: string = `https://api.openweathermap.org/data/2.5/weather?zip=${zipcode},us&appid=${API_Key}&units=imperial`;
    fetch(fetchURL)
      .then(response => response.json())
      .then(data => {
